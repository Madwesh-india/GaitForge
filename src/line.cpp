...
{
    end_point_2 = _length*std::exp(ComplexD(0, 0));
...
bool Line::update()
{
    update_curve_points_1 = ((_context._m_2_pxl_scale * std::exp(ComplexD(0, M_PI_2))) * (const_curve_points_1)).array().conjugate() + offset; 
    update_curve_points_2 = ((_context._m_2_pxl_scale * std::exp(ComplexD(0, M_PI_2))) * (const_curve_points_2.array() + end_point_2)).conjugate() + offset;
    return true;
}
...
bool Line::draw()
{
    for (int i=0;i<CURVE_RESOLUTION;i++){
        _verts[i].position.x = update_curve_points_1[i].real();
        _verts[i].position.y = update_curve_points_1[i].imag();

        _verts[i + CURVE_RESOLUTION].position.x = update_curve_points_2[i].real();
        _verts[i + CURVE_RESOLUTION].position.y = update_curve_points_2[i].imag();
